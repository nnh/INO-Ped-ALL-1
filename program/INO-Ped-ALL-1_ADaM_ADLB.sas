...
*
* Program name      : INO-Ped-ALL-1_ADaM_ADLB.sas
*
...
data  wk10;
  ...
  if  missing(LBORRESU) then do;
    PARAM=strip(LBTEST);
    AVALC=strip(LBORRES);
  end ;
  else if  ^missing(LBORRESU) then do;
    PARAM=strip(LBTEST)||" ("||strip(LBORRESU)||")";
    AVAL=input(LBORRES,best32.);
  end ;
...
data  wk41;
  set  wk30;
  RENAME AVAL=BASE;
  keep USUBJID PARAMCD AVAL ADY;
  if ADY<=1;
run ;

proc sort data=wk41; by USUBJID PARAMCD decending ADY; run ;
proc sort data=wk41 out=wk40(drop=ADY) nodupkey; by USUBJID PARAMCD; run ;
