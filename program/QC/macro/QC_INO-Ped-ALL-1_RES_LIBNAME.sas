...
%macro EDIT_MEANS_2(input_ds, output_ds, target_var);
    ...
    data &output_ds.;
        ...
        * N;
        &output_var.=n; output;
        * MEAN;
        &output_var.=put(round(temp_mean, &round_mean.), &put_mean.); output;
        * SD;
        if temp_sd^=. then do;
          &output_var.=put(round(temp_sd, &round_sd.), &put_sd.); output;
        end;
        else do;
          &output_var.='-'; output;
        end;
        ...
        * MEDIAN;
        &output_var.=put(round(temp_median, &round_median.), &put_median.); output;
        * MAX;
        if missing(&round_max.) then do;
          ...
          if max^='-0.0' then do;
            &output_var.=put(round(max, &round_max.), &put_max.); output;
          end;
          else do;
            &output_var.='0.0'; output;
          end;
    ...
    run;
%mend EDIT_MEANS_2;
%macro EDIT_ROUND_TABLE(mean=0.01, sd=0.001, min=0.1, median=0.01, max=0.1);
    data temp_round_table;
        length title $200;
        title='mean';
        round=&mean.; output;
        title='sd';
    ...
    run;
    data round_table;
        set temp_round_table;
        if round=0.1 then do;
          put=8.1;
        end;
        else if round=0.01 then do;
          put=8.2;
        end;
        else if round=0.001 then do;
          put=8.3;
        end;
        else do;
          put=8.1;
