...
*
* Program name      : INO-Ped-ALL-1_ADaM_ADRS.sas
*
* Author            : MATSUO YAMAMOTO
*
* Date created      : 20201225
*
* Purpose           : Create ADRS DataSet
*
* Revision History  : 
*
...
data  wk31;
  ...
  select(AVALC);
    ...
    when("CRi") seq=2;
    when("PR") seq=3;
    when("RESISTANT DISEASE") seq=4;
    when("DEATH DURING APLASIA") seq=5;
    when("RELAPSED DISEASE FROM CR OR Cri") seq=6;
    when("PD") seq=7;
    when("INDETERMINATE RESPONSE") seq=8;
    otherwise;
  end;
run ;

proc sort data=wk31 out=wk30; by USUBJID seq AVISITN; run ;
proc sort data=wk30(drop=seq) nodupkey; by USUBJID; run ;
...
data  wk40;
  ...
  PARAMCD="DLT";
  AVALC="N";
  keep USUBJID PARAM PARAMCD AVALC ;
run ;

/* set */
data  wk110;
  length AVISIT $200.;
  set  wk10 wk20 wk30 wk40;
  select (AVISITN);
